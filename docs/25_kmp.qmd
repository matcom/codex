...
The cynosure for this chapter is one sentence: **the failure function tells you where to resume; the text pointer never moves backward.** That sentence is the entire algorithm in compressed form. Failure function, $m$ integers. Text pointer, one variable that strictly increases. By the end of this chapter you'll have built the failure function for any pattern in a single linear pass, run KMP on the canonical text from chapters 23 and 24 to recover the same four matches of `"ain"`, watched the failure function for `"ababa"` and `"abracadabra"` produce non-trivial fallback behaviour you can trace by hand, and — the punchline — reconstructed every cell of chapter 24's DFA from the failure function alone, cell by cell, proving that the DFA and KMP are the same machine in two storage formats. The compression is lossless. KMP isn't a different algorithm from the DFA; it's the DFA stored in $\Theta(m)$ space instead of $\Theta(m \cdot |\Sigma|)$.
