...
That trade — accepting probabilistic answers in exchange for vastly less memory — is what makes Bloom filters useful in the kinds of places where memory is genuinely the bottleneck. Web crawlers use them to remember which URLs they've already fetched without storing a hash table of every URL. Databases use them to skip disk reads on queries that "definitely-not" hits the database. Distributed systems use them to summarize what's on each node so other nodes can avoid expensive remote lookups for keys that aren't there.

By the end of this chapter you'll have built a Bloom filter from a bit array and $k$ hash functions, derived the false-positive formula $\varepsilon = (1 - e^{-kn/m})^k$ from first principles, computed the optimal $k$ and $m$ given a target false-positive rate, and empirically verified that the formula predicts reality. The cynosure to hold is **the price of compression is doubt**. Bloom filters compress a set by losing the ability to enumerate its members — and that loss is *what buys* the space efficiency. You cannot ask a Bloom filter "what's in you?" You can only ask "is this in you?" — and the answer has a known error rate on the side that's allowed to be wrong.

## The setting: huge set, tight memory
...
If any of the $k$ bits is 0, the answer is *definitely-not* — because if $x$ had been added, all $k$ of its bits would have been set, and zero of them would still be zero. If all $k$ are 1, the answer is *probably-yes* — those bits might all be 1 because $x$ was added, or because other items collectively set the same bits. The second case is a false positive, and its probability is what the next section computes.

The "$k$ independent hash functions" requirement is awkward in practice — you'd have to implement $k$ separate hash functions and tune them not to be correlated. The textbook trick (Kirsch and Mitzenmacher, 2006) is that you only need *two* independent hash functions $h_1$ and $h_2$, and then $h_i(x) = h_1(x) + i \cdot h_2(x) \pmod m$ for $i = 0, 1, \ldots, k-1$ behaves indistinguishably from $k$ independent hashes for the Bloom-filter analysis. That's the double-hashing trick from chapter 12, repurposed.

## False positives, derived
...
The approximation uses $\lim_{m \to \infty} (1 - 1/m)^{m} = 1/e$. For Bloom filters where $m$ is in the thousands at minimum, the approximation is excellent.

Probability that a specific bit *is* 1: $1 - e^{-nk/m}$.

Probability that a *false positive* happens — that all $k$ bits checked by a query for an item *not* in the set are 1 — assuming the $k$ probed positions are independent:

$$\varepsilon = \left(1 - e^{-nk/m}\right)^k$$

That's the false-positive rate formula. It depends on $n$ (how many items you've added), $m$ (how big the bit array is), and $k$ (how many hashes per item). $n$ is fixed by the workload; $m$ and $k$ are design parameters.

The bilingual restatement: **more hashes hurt past a point**, because more hashes mean more bits get set per insertion, which means the bit array fills up faster, which means more false positives downstream. The optimal $k$ minimizes the false-positive rate for a given $n$ and $m$.
...
The bit array lives inside a `bytearray` — a mutable sequence of 8-bit integers. To set bit $i$, you compute the byte index $i // 8$ and the bit-within-byte $i \bmod 8$, then OR in `1 << bit`. To test bit $i$, you AND with the same mask and check whether the result is nonzero. That's the standard bit-array implementation idiom.
...
Empirical rate within a fraction of a percent of the predicted 1%. The agreement isn't an accident — the analysis is exact (modulo the $(1 - 1/m)^m \to 1/e$ approximation, which is tight at this scale), and the only randomness in the experiment is from the choice of test strings, which is exactly the regime the formula models.
...
The predicted false-positive rate sits right on the target every time — which is the whole point of choosing $m$ and $k$ from the closed-form formulas. The structure is parametric, not heuristic.
...
**Enumeration isn't supported.** There's no `for x in bf` operation, and there couldn't be — the keys themselves were never stored, and the bit pattern alone can't reconstruct them. The structure can answer "do you contain this specific thing?" but never "what do you contain?" That asymmetry is what the cynosure was naming: the price of compression is doubt, and the doubt extends specifically to recovering the members.
...
## What Bloom filters teach you

The Bloom filter trades exact membership testing for a probabilistic version of the same operation, and in exchange uses an order of magnitude less memory than a structure that stores the keys themselves. It's correct in the one-sided sense — no false negatives, false positives at a tunable rate. It runs in $\Theta(k)$ per operation and uses $\Theta(n \log(1/\varepsilon))$ bits, which is within a constant factor of the information-theoretic lower bound.

The lesson worth carrying forward is the design move: **trade certainty for space**. The structure achieves its memory efficiency precisely *by* giving up the ability to enumerate or reconstruct what it holds. The trade isn't a bug — it's the point. Asymmetric error guarantees (definite negatives, probable positives) are the contract that makes the trade work, because the asymmetry lets the structure be useful as a pre-filter even when the positives have a known error rate.

This pattern shows up across the probabilistic-data-structures family in Part IX. Count-Min sketch trades exactness for space when *counting* frequencies of items in a stream. HyperLogLog trades exactness for space when *counting distinct items* in a stream — and gets the count of a billion-element multiset accurate to within 2% using a kilobyte of memory. All of them are different versions of the same move: identify exactly what the workload needs to know, and design a structure that stores only *that*, with a known error rate on the side you're allowed to be wrong.

You've now seen six data structures across this part. The last chapter introduces a structure whose contract isn't "store a value" or "check membership" — it's "maintain a partition." And the algorithm that makes it work is so close to optimal that describing its time complexity requires the inverse Ackermann function. After hash tables and Bloom filters, that's the right note for Part II to end on.
