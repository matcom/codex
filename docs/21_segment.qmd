...
By the end of this chapter you'll have built a **segment tree**: a binary tree where every node owns a sub-interval of the array and stores the aggregate of the values in that interval. You'll have point-update plus range-query in $O(\log n)$ per operation, and you'll have the harder case (range-update plus range-query) solved with **lazy propagation**, a technique that defers work until somebody actually asks for it.

The cynosure to hold: **every node owns an interval, and the answer to any range query is the sum of the $O(\log n)$ canonical intervals that tile it.** A segment tree is what you get when you stop thinking about array indices and start thinking about intervals as first-class objects.

## The range-query problem

Here's the problem. You have an array of $n$ numbers — call them $a_0, a_1, \ldots, a_{n-1}$ — and you need to support two operations in any order, any number of times.
...
For a mixed workload of comparable updates and queries, both approaches degrade to $O(n)$ amortized per operation, which is the cost of doing nothing clever at all. The segment tree gives you $O(\log n)$ for **both**. Bilingually: on an array of a million elements, both update and query cost about twenty steps; on a billion, about thirty.

## The segment-tree layout

I'll think of the array's index range $[0, n)$ as one big interval, recursively split in half. The root node owns $[0, n)$. Its left child owns $[0, n/2)$; its right child owns $[n/2, n)$. The recursion bottoms out at intervals of length one, where each leaf owns one array position and stores its value.
...
The cynosure shows up here. A range query like "sum of $[3, 11)$" doesn't visit every leaf in $[3, 11)$ — that would be $O(n)$. It visits at most $O(\log n)$ internal nodes whose intervals together *tile* $[3, 11)$ exactly. Each visited node returns its stored aggregate in $O(1)$, and I sum them. The tiling is what makes the query logarithmic.

## Point update, range query

I'll build the structure top-down, then add update and query. The constructor takes the data and an aggregation `op` with its identity element (0 for sum, $-\infty$ for max, etc.).
...
You can see the walk visits exactly one node per level, descending into the half containing $i$ and ignoring the other. Tree height is $O(\log n)$, so update is $O(\log n)$ time and $O(\log n)$ stack frames. Bilingually: updating one position in a million-element tree touches about twenty nodes.

Now the range query. To compute the aggregate over $[l, r)$ from a node owning $[lo, hi)$, three cases come up. **Disjoint**: the two intervals don't overlap, so the node contributes nothing; return the identity. **Contained**: the node's interval lies entirely inside the query, so the stored aggregate is exactly what I want. **Crossing**: partial overlap, so recurse on both children and combine.
...
Demonstrating on a power-of-two array of eight values, with queries before and after a point update.
...
I won't implement this variant separately — the next section's lazy propagation subsumes it. Build the lazy version and call `query(i, i+1)` and you get the point-query semantics for free.
...
The invariant I'll maintain: **`tree[node]` is the correct aggregate for the node's interval, accounting for all lazy values at this node and at every ancestor, but not yet for any lazy values stored at descendants.** Every operation that descends through a node pushes its lazy value down first.
...
I'll demo range-add plus range-sum on a fresh array of eight ones.
...
Bilingually: on a million-element array both operations touch about $4 \log_2(10^6) \approx 80$ tree nodes; on a billion, about 120. Storage is $\Theta(n)$: the $4n$ over-allocation absorbs index slack for non-power-of-two $n$, and the lazy variant doubles that with the `_lazy` array.
...
## What segment trees teach you

Every node owns an interval, and the answer to any range query is the sum of the $O(\log n)$ canonical intervals that tile it. That's the cynosure, and once it lands the rest is bookkeeping. Three lessons for you to carry forward. **First, the right abstraction is sometimes an interval, not an index.** The naive approaches think about indices; the segment tree gives every interval a node, and range queries become one lookup instead of $n$. **Second, laziness is a data-structure technique, not just a programming-language one.** A range update marks $O(\log n)$ tiles and stops; the marks only propagate when a subsequent query forces them. The same idea powers garbage collection, copy-on-write file systems, and database query planners. **Third, the layout is the algorithm.** A segment tree is a flat array under index arithmetic, chapter 19's trick again. What changes is what each slot stores. The implicit-binary-tree-over-flat-array pattern has been the backbone of three consecutive chapters; chapter 22 makes it four.
