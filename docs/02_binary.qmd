...
This is binary search, and by the end of this chapter you'll have implemented it three ways: as a straight "is $x$ in the sequence?" answer, as a *bisection* that returns an insertion point, and — most usefully — as a way to search the answer space of any problem with monotonic structure. That third move is the one that keeps paying off through the rest of the book.

The image to hold in your head is **every comparison cuts the search space in half**. For a sorted list of one million items, that's about twenty comparisons to find anything; for a billion items, about thirty. Doubling the input adds *one comparison*, not doubles the time. Linear search would scan all billion items. Binary search reads thirty of them and tells you the answer with certainty. That's the leap structure buys you.
...
Two details worth dwelling on. First, `mid = (lo + hi) // 2` uses integer division, which always rounds toward `lo` — that's important for termination, because it guarantees `mid < hi` whenever `lo < hi`, so the `lo = mid + 1` branch makes real progress. Second, the input *must* be sorted in the order that `order` defines. If you pass an unsorted list, binary search gives you garbage with confidence. There's no easy way for the algorithm to detect this — sorting is a precondition, not something the function checks. If you want a safety net, sort first and document the contract loudly.
...
The pair `(bisect_left, bisect_right)` doesn't just answer *where to insert*. It also gives you, almost for free, the **count** of an element in a sorted sequence — just subtract: `bisect_right(x, items) - bisect_left(x, items)`. That's $O(\log n)$ instead of the $O(n)$ count you wrote in the last chapter. Structure pays off twice.

## Searching the answer, not the data

Now the move that pays off through the rest of the book.
...
## What binary search teaches you
...
In the next chapter, the problem flips. So far you've been *exploiting* order to search faster. The next question is how to *create* that order in the first place — and the answer is going to take three chapters of escalating cleverness before it lands at the comparison lower bound for sorting.
